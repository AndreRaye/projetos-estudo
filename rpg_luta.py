...
Você escolhe qual habilidade gostaria de usar, o programa então calcula o dano da sua habilidade e se ela é um acerto crítico.
O programa escolhe automaticamente qual será a habilidade do inimigo e o dano de acerto dela.
O programa acaba quando a sua vida ou a do inimigo chegam a zero.
"""
from random import *
...
print("-"*80)
print("Bem-vindos!\nEste programa é um jogo de luta no estilo RPG baseado em turnos.\nO jogo acaba quando seu PV (pontos de vida) ou do inimigo chegam a zero.")
...
while pv1 >= 0 and pv2 >= 0:
    ...
    if habilidade == '1':
        ...
        dano2 = dano * 2
        if dado == 20:
            print(f"\nACERTO CRÍTICO!\nVocê usou o kage bunshin no jutsu, tirando um total de {dano2} PV.")
            pv2 = pv2 - dano2
            print(f"PV do inimigo: {pv2}")
        else:
            print(f"\nVocê usou o kage bunshin no jutsu, tirando um total de {dano} PV.")
            pv2 = pv2 - dano
            print(f"PV do inimigo: {pv2}")
        ...
        dano2 = dano * 2
        if dado == 20:
            print(f"\nACERTO CRÍTICO!\nVocê acertou o inimigo com um rasengan, tirando um total de {dano2} PV.")
            pv2 = pv2 - dano2
            print(f"PV do inimigo: {pv2}")
        else:
            print(f"\nVocê acertou o inimigo com um rasengan, tirando um total de {dano} PV.")
            pv2 = pv2 - dano
            print(f"PV do inimigo: {pv2}")
